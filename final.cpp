...
int binarySearchTrain(const vector<data>& trains, int trainno) {
...
void bubbleSortByFare(vector<data>& trains) {
...
int reservation1()
{
    ...
    vector<data> s(15); // Note: C code used s[1] to s[14], C++ vector is 0-indexed. Adjusting loop and access.
    int c,d,i,j;
    ...
    int trainIndex = binarySearchTrain(s, f);

    h = d * s[trainIndex].fare;
    far = h;
    ...
    if(e == 1) {
        ...
        // Find return train index
        int returnTrainIndex = binarySearchTrain(s, sa);

        // Add return fare
        h = h + d * s[returnTrainIndex].fare;
    ...
    else // No return ticket
    {
        ...
        cout << "\n\n******************************************";
        cout << "\n* YOUR TOTAL FARE IS " << h << " + " << gs1 << " = " << h + gs1 << " *";
        cout << "\n******************************************";
        cout << "\nPRESS 1 WHEN YOU COMPLETED THE PAYMENT ";
        cin >> fj;
